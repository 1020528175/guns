...
### Guns v5.0更新内容
> * 剔除了shiro，集成自研单点登陆框架，详情见`sso`分支（shiro版本还是在master分支）
...
#### 1.执行guns-admin模块下sql/guns.sql初始化数据库
#### 2.执行guns-sso-server模块下sql/sso_server.sql初始化数据库
#### 3.执行guns-sso-server模块下的SsoServerApplication类下的main方法启动sso单点登陆服务器
#### 4.执行guns-admin模块下的GunsApplication类下的main方法，启动guns-admin后台管理系统，登陆地址是：localhost:8080
#### 5.执行guns-example-client模块下的SsoClientExampleApplication类下的main方法，登陆地址是：localhost:8081，启动单点登陆客户端的演示程序，其实guns-admin也是一个单点登陆客户端
#### 6.打开localhost:8080准备登陆进guns系统，系统会自动跳转到sso服务器，数据默认账号密码：admin/admin，系统登陆guns管理系统成功
#### 7.打开localhost:8081准备进入论坛，系统不会跳转到登陆，直接进入论坛，单点登陆演示成功！
